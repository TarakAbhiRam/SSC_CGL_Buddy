...
def build_prompt(
    ...
    if topics:
        topic_line = (
            "\nFocus specifically on these subtopics (spread the questions across them): "
            + ", ".join(topics) + "."
        )
...
def validate_mcq(item: Any) -> Optional[Dict[str, Any]]:
...
def generate_mcqs(
    ...
    try:
        ...
        raw_retry = _call_provider(prompt + "\n\nReturn ONLY a valid JSON array.", provider, api_key)
        return parse_mcqs(raw_retry)
